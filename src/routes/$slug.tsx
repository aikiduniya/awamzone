...
export const Route = createFileRoute("/$slug")({
  ...
  notFoundComponent: () => (
    <SiteShell>
      <div className="container-luxe py-20 text-muted-foreground">Page not found.</div>
    </SiteShell>
  ),
  errorComponent: () => (
    <SiteShell>
      <div className="container-luxe py-20 text-muted-foreground">Unable to load page.</div>
    </SiteShell>
  ),
  component: CmsPage,
});
...
function CmsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("pages").select("*").eq("slug", slug).eq("is_published", true).maybeSingle()).data,
  });
  return (
    <SiteShell>
